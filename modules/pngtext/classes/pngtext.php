...
class PNGText {
    ...
    public function __construct($text, $styles = array()) {
        ...
        if (empty($this->_styles['line-height']) or $this->_styles['line-height'] == 'normal') {
            // Default line-height
            $this->_styles['line-height'] = $this->_styles['font-size'] * 1.5;
        } else {
        ...
        if (empty($this->_styles['color'])) {
            // Use pure black
            $this->_styles['color'] = array(0,0,0,10);
        } else {
            // Scan rgb(r, g, b)
            $this->_styles['color'] = sscanf($this->_styles['color'], 'rgb(%d, %d, %d)');
            array_push($this->_styles['color'], 10);
        }
    ...
    public function _process() {
        ...
        for($i = 0; $i < count($lines); $i++) {
            ...
            $this->_max_width = max($width+6, $this->_max_width);
