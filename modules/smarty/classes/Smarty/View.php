...
class Smarty_View extends View {
    ...
    public function &__get($key) {
        ...
        if ($var instanceof Undefined_Smarty_Variable) {
            throw new Kohana_Exception('View variable is not set: :var', array(
                ':var' => $key 
            ));
        } else {
            return $var->value;
    ...
    private static function create_dir($path, $name = '') {
        if (file_exists($path)) {
            if (is_dir($path)) {
                throw new Kohana_Exception('Could not write to :name directory', array(
                    'name' => $name 
                ));
            } else {
    ...
    public function set_filename($file) {
        if (!View::is_smarty_template($file)) {
            throw new Kohana_Exception('Cannot use set_filename to change from a Smarty template to :tpl.', array(
                ':tpl' => $file 
            ));
        }
